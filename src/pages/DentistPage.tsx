        ...
        data={[
          {
            ...
            items: [
              { name: 'Home', url: '/' },
              ...(stateSlug && citySlug ? [{ name: dentist.clinic?.city?.state?.name || '', url: `/${stateSlug}/` }] : []),
              ...(stateSlug && citySlug ? [{ name: cityName, url: `/${stateSlug}/${citySlug}/` }] : []),
              { name: dentist.name },
            ],
          },
          {
            ...
            jobTitle: dentist.title || 'Dentist',
            description: dentist.bio || `${dentist.name} is a dental professional listed on AppointPanda. Find reviews, contact information, and book appointments.`,
            image: dentist.image_url || undefined,
            url: `https://www.appointpanda.com/dentist/${dentist.slug}/`,
            worksFor: dentist.clinic ? {
              '@type': 'DentalClinic',
              name: dentist.clinic.name,
              url: `https://www.appointpanda.com/clinic/${dentist.clinic.slug}/`,
            } : undefined,
            address: dentist.clinic?.address ? {
              '@type': 'PostalAddress',
              streetAddress: dentist.clinic.address,
              addressLocality: dentist.clinic.city?.name,
              addressRegion: dentist.clinic.city?.state?.abbreviation,
              postalCode: dentist.clinic.zip,
              addressCountry: 'US',
            } : undefined,
            geo: dentist.clinic?.latitude && dentist.clinic?.longitude ? {
              '@type': 'GeoCoordinates',
              latitude: dentist.clinic.latitude,
              longitude: dentist.clinic.longitude,
            } : undefined,
            telephone: dentist.clinic?.phone,
            aggregateRating: dentist.rating ? {
              '@type': 'AggregateRating',
              ratingValue: Number(dentist.rating),
              reviewCount: dentist.review_count || 0,
            } : undefined,
            medicalSpecialty: dentist.specializations || [dentist.title || 'General Dentistry'],
            availableService: dentist.specializations?.map(spec => ({
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {dentist.languages && dentist.languages.length > 0 && (
              <div className="card-modern p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {dentist.languages.map((lang, i) => (
                    <Badge key={i} variant="secondary" className="rounded-full px-4 py-1 font-medium">
                      {lang}
                    </Badge>
                  ))}
                </div>
            ...
            {dentist.specializations && dentist.specializations.length > 0 && (
              <div className="card-modern p-6">
                <h2 className="font-display text-xl font-bold mb-4">Specializations</h2>
                <div className="flex flex-wrap gap-2">
                  {dentist.specializations.map((spec, i) => (
                    <Badge key={i} variant="outline" className="rounded-full px-4 py-1 font-medium">
                      {spec}
                    </Badge>
                  ))}
                </div>
              ...
              <div className="card-modern p-6">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {treatments.map((dt) => (
                    <Link
                      key={dt.id}
                      to={`/services/${dt.treatment?.slug}`}
                      className="flex items-center gap-3 p-4 rounded-xl bg-muted/50 hover:bg-muted transition-colors"
                    >
                      <span className="text-2xl">{dt.treatment?.icon || "🦷"}</span>
                      <span className="font-bold">{dt.treatment?.name}</span>
                    </Link>
                  ))}
                </div>
              </div>
